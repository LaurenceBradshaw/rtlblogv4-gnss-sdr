      ...
      constellation_( sig.constellation )
{
    ...
    nfreq_           = 2 * static_cast<int>( Signal_aiding::SEARCH_HBAND_WIDE_HZ / doppler_step_hz_ ) + 1;
...
bool Acquisition_engine::integrate( const Sample_block& block )
{
    ...
    if( intg_count_ == 0 )
    {
        const Signal_aiding::Estimate est = aiding_.estimate( constellation_, satellite_id_, carrier_freq_hz_ );
