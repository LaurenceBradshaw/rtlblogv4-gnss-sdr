...
{
    return std::make_unique<Costas_tracker>( code_chips( sv ), sample_rate_hz, params(), secondary_code( sv ) );
}
...
std::unique_ptr<Signal> make_signal( Constellation constellation, Band band, Code code )
{
    ...
    if( constellation == Constellation::Gps && band == Band::L1 && code == Code::Cd )
        return std::make_unique<Gps_l1c_signal>();
